#!/usr/bin/env python3
"""
Adds Brian2 simulator repositories to neuromorphic_repos.json,
clones them into data/github_repos/brian2/, and rebuilds code_index.jsonl.

Fetches:
- Core Brian2 packages (brian-team org)
- Dendrify (Poirazi-Lab) — multi-compartment LIF models built on Brian2
- Any repos that implement SNN models or extensions using Brian2

Run from repo root OR dataset-assembly/scripts/code/:
    python dataset-assembly/scripts/code/add_brian2.py
"""
...
class GitHubRepoFetcher:
    ...
    def search_repositories(self, query: str, max_results: int = 15) -> List[Dict]:
        repos: List[Dict] = []
...
def main():
    ...
    for category, cfg in CATEGORIES.items():
        ...
        max_per_query = cfg.get("max_per_query", 15)
    ...
    for category, cat_repos in all_new.items():
        ...
        print(f"\n{'='*60}")
        print(f"Cloning {category} repos into {dest}")
        print(f"{'='*60}")
